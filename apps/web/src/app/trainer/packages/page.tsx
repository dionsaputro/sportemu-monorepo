    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">Kelola paket latihan & harga kamu</p>
        </div>
        <Link
          href="/trainer/packages/new"
      ...
      {!packages || packages.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white py-16 text-center">
          ...
          <Link href="/trainer/packages/new" className="mt-4 inline-flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-sky-700 hover:bg-sky-50">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {packages.map((pkg: any) => (
            <div key={pkg.id} className="rounded-xl border border-slate-200 bg-white p-5 transition-shadow hover:shadow-sm">
              ...
              <p className="mt-3 text-xl font-bold text-sky-900">{formatPrice(pkg.price)}</p>
